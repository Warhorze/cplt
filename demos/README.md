...
3. Each `_sim.sh` sources `lib.sh` for helpers and simulates a user building a csvplot command

## Simulation Rules

The sim scripts follow strict patterns to make demos look like a real user discovering csvplot's features.
...
echo "female  male"
printf '%s' "$PROMPT"
printf '%s' "csvplot bar ... --where Sex="
sleep 2
...
The lib also adds `.venv/bin` to PATH so `csvplot` is available without manual activation.
...
Requires [VHS](https://github.com/charmbracelet/vhs) and csvplot installed in the project venv.
